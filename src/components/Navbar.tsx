  ...
  return (
    <header className="bg-white shadow-sm sticky top-0 z-50">
      <div className="container-custom mx-auto px-1 sm:px-2 py-1 md:py-3">
        <nav className="flex justify-between items-center min-h-[44px] md:min-h-[56px]">
          {/* Logo */}
          <Link
            to="/"
          ...
          >
            <img
              ...
              alt={t('common.brand')}
              className="h-7 w-auto md:h-16 md:max-h-16 max-w-[92px] sm:max-w-[160px] object-contain"
              draggable="false"
              style={{ maxHeight: 40 }}
            />
          </Link>
          ...
          <div className="md:hidden flex items-center">
            <Link
              to="/cart"
              className="relative mr-1 md:mr-2 p-1 rounded focus:outline-none focus:ring-2 focus:ring-brand-blue min-w-11 min-h-11"
              aria-label={t('navigation.cart')}
              tabIndex={0}
            >
              <ShoppingCart className="h-7 w-7 text-gray-700 hover:text-brand-blue min-w-11 min-h-11" />
              {cartItems.length > 0 && (
            ...
            <button
              ...
              className="text-gray-700 hover:text-brand-blue focus:outline-none p-1 rounded-full active:bg-gray-100 transition min-w-11 min-h-11"
        ...
        {isMenuOpen && (
          <>
            ...
            <div
              ref={menuRef}
              className="fixed inset-0 z-[110] bg-white py-3 px-2 flex flex-col rounded-t-3xl rounded-b-none shadow-2xl animate-slide-in-up transition-all duration-300
            ...
            >
              ...
              <button
                onClick={() => setIsMenuOpen(false)}
                className="absolute top-2 right-2 text-gray-600 hover:text-brand-blue rounded-full p-1.5 focus:outline-none focus:ring-2 focus:ring-brand-blue"
                aria-label={t('navigation.closeMenu', 'Close menu')}
                tabIndex={0}
              >
                <X className="h-7 w-7" />
              </button>
              <nav className="flex flex-col mt-14 gap-2 w-full" aria-label="Mobile Navigation">
                {navLinks.map((link) => (
                  <Link
                    key={link.path}
                    to={link.path}
                    className={`
                      block w-full py-2 px-3 rounded-xl text-base font-semibold text-left transition-all
                      ${isActive(link.path)
